...
qboolean Pickup_Weapon (edict_t *ent, edict_t *other)
{
	...
	// BEGIN CTC
	if (other->client && other->client->pers.inventory[chickenItemIndex] > 0 && !other->client->isThrowing)
		return false;
	else
	{ // Added upto here and the single bracket below.
...
void ChangeClipIndex (edict_t *ent)
{
	...
	// BEGIN CTC - What am I doing?
	else if (!strcmp(ent->client->pers.weapon->pickup_name, "Chicken"))
		ent->client->clip_index = CLIP_CHICKEN;
	// END
}
...
int QweryClipIndex (gitem_t *item)
{
	...
	// BEGIN CTC - Totally unsure about this stuff.
	else if (!strcmp(item->pickup_name, "Chicken"))
		return CLIP_CHICKEN;
	// END
