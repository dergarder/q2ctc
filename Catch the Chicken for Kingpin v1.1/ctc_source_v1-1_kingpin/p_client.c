...
extern void CheckForCheating(edict_t *ent);
// END
...
void ClientObituary (edict_t *self, edict_t *inflictor, edict_t *attacker)
{
	...
	// BEGIN CTC
	if (Chicken_PlayerDie(self, inflictor, attacker, meansOfDeath)) return;
	// END
	...
	if (deathmatch->value || coop->value)
	{
		...
		case MOD_TARGET_BLASTER:
			message = "got blasted";
			break;
		case MOD_CHICKEN:						// BEGIN CTC
			message = "got pecked to death";
			break;								// END
...
//	self->solid = SOLID_NOT;
	...
	if (!self->deadflag && (self->health + damage > 0))
	{
		...
		// BEGIN CTC
		if (!chickenDropped) TossClientWeapon (self);
// Commented line below out as the above one replaces it ????
...
// done.
	...
	// BEGIN CTC - This code is very suspect as its quite a bit different from the
	//             original code.
	if ((damage >= 50 && self->health < -30 && !inflictor->client) ||
		(meansOfDeath == MOD_CHICKEN || self->client->isThrowing))
	{	// gib
		GibEntity( self, inflictor, damage );
		self->s.renderfx2 |= RF2_ONLY_PARENTAL_LOCKED;
	}
	else // BEGIN CTC - Added this else as it was in the Q2 CTC source ???
	{	// normal death
		if (!self->deadflag)
		{
			static int i;
...
//	ent->client->team = team;
	...
	if (deathmatch->value)
		Chicken_CheckInPlayer(ent);

	ent->client->gotChickenModel	= -1;

//	if (Chicken_PlayerReadyEggGun(ent, teamWithChicken))
//		ChangeWeapon (ent);
...
void ClientBeginDeathmatch (edict_t *ent)
{
	...
	// BEGIN CTC
//	Chicken_ClientBegin(ent); // MOTD stuff so we aren't bothered about it :)
	// END
...
void ClientDisconnect (edict_t *ent)
{
	...
	{
	case 0:
		gi.bprintf (PRINT_HIGH, "%s flew the coup\n", ent->client->pers.netname);
		break;
	case 1:
		gi.bprintf (PRINT_HIGH, "%s checked out. Chicken!\n", ent->client->pers.netname);
		break;
	}
...
//	Chicken_CheckOutPlayer(ent); // Appears to be team leaving type stuff
...
car_resume:
	...
	if (client->latched_buttons & BUTTON_ATTACK /* && ent->movetype != MOVETYPE_NOCLIP*/) // BEGIN CTC
	{
