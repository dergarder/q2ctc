...
//	int		picnum;
	int		/*x,*/ y;
	gclient_t	*cl;
	edict_t		*cl_ent, *chickenholder=NULL, *nextplayer=NULL;
	char	*tag;
...
//#if ENABLE_INDEX_NAMES && false
	if (ent->client->pers.version >= 114)
	{
		...
		{
			Com_sprintf (entry, sizeof(entry),
				"xr %i yv %i dmstr 862 \" Name            Ping Time Score Klls\" ",
				-36*10 - 15, -60+-2*14 );
		}
	}
//#else
...
//		picnum = gi.imageindex ("i_fixme");
	...
	// BEGIN CTC
	// Set the name of the Chicken holder on the display.
	while ((nextplayer = G_Find (nextplayer, FOFS(classname), "player")) != NULL)
		{
		if (nextplayer->client->pers.inventory[chickenItemIndex] > 0)
			{
			chickenholder = nextplayer;
			break;
			}
		}
...
//#if ENABLE_INDEX_NAMES && false
		if (ent->client->pers.version >= 114)
		{
			...
			{	// BEGIN CTC 
/*				if ((chickenholder != NULL) && (chickenholder->client == cl))
					Com_sprintf (entry, sizeof(entry),
						"yv %i ds %s %i %i %i %i xr %i dmstr %s \"-->\" ",
						-60+i*16, tag, sorted[i], cl->ping, (level.framenum - cl->resp.enterframe)/600, cl->resp.score, -36*10 - 40,tag );
				else
					Com_sprintf (entry, sizeof(entry),
						"xr %i yv %i ds %s %i %i %i %i ",
						-36*10 - 10,-60+i*16, tag, sorted[i], cl->ping, (level.framenum - cl->resp.enterframe)/600, cl->resp.score );
*/				// END
				if ((chickenholder != NULL) && (chickenholder->client == cl))
					Com_sprintf (entry, sizeof(entry),
						"yv %i dmstr %s \"%-16s %4i %3i  %4i %3i\" xr %i dmstr %s \"-->\" ",
						-60+i*16, tag, cl->pers.netname, cl->ping, (level.framenum - cl->resp.enterframe)/600, cl->resp.score, cl->resp.kills, -36*10 - 50,tag );
				else
					Com_sprintf (entry, sizeof(entry),
						"xr %i yv %i dmstr %s \"%-16s %4i %3i  %4i %3i\" ",
						-36*10 - 15, -60+i*16, tag, cl->pers.netname, cl->ping, (level.framenum - cl->resp.enterframe)/600, cl->resp.score, cl->resp.kills );
			}
		}
//#else
		else
...
//#endif
	...
	// BEGIN CTC - Just to let people know what the '*' means.
	Com_sprintf (entry, sizeof(entry),
		"xr %i yv %i dmstr 999 \"--> Denotes Chicken Holder\" ",-36*10 - 40,  -60+((i+1)*16) );
...
extern void ShowCTC_Config(edict_t *ent);
...
void G_SetStats (edict_t *ent)
{
	...
	*/
	...
	// BEGIN CTC
	//
	// Display your position out of the number of players.
