...
void KickCheater( char *twat );

void Vote_Respawn( edict_t *ent );
void RespawnThink(edict_t *ent);
qboolean EnoughRespawnVotes(void);

extern int Chicken_Respawn(void *data, int itemId);
...
// BEGIN CTC
qboolean	ChickenVote;
// END

void Cmd_Players_f (edict_t *ent);
void Cmd_RespawnResponse_f (edict_t *ent);
...
//******************************************************************************************
// Check to see what type of vote is required and start it.
//******************************************************************************************
void Cmd_Vote_f (edict_t *ent)
	...
	// BEGIN CTC - Start a vote for respawning the chicken.
...
//******************************************************************************************
// Start the respawn chicken going and tell everyone to start voting.
//******************************************************************************************
...
void Vote_Respawn( edict_t *ent )
	...
	gi.cprintf( ent, PRINT_HIGH, "You have voted to respawn the chicken.\nAwaiting responses from other players.\n" );

	gi.dprintf( "%s formed a vote to respawn the chicken.\n", ent->client->pers.netname );
		
	// start the thinker
	target = G_Spawn();
	target->owner = ban_target;
	target->think = RespawnThink;
	target->nextthink = level.time + 15;
	target->count = ban_id;

	ban_thinker = target;

	// This is so the response command gets processed by another function.
	ChickenVote = true;
	...
	for (i=1; i<=(int)maxclients->value; i++)
		...
		gi.cprintf( &g_edicts[i], PRINT_CHAT, ">> %s has voted to respawn the chicken. Type 'yes' in the console if you agree, 'no' if you disagree. %i votes required.", ent->client->pers.netname, ban_player_count );
		}
	}


//******************************************************************************************
// Check for enough votes and respawn the chicken if successful
//******************************************************************************************
void RespawnThink(edict_t *ent)
{
	...
	// if we didn't have enough, cancel the kick request
	if (!EnoughRespawnVotes())
		{
		if (ban_source->inuse)
			gi.cprintf( ban_source, PRINT_HIGH, "Vote to respawn the chicken did not succeed.\n" );
	...
	// Respawn the chicken
	Chicken_Respawn(NULL, 1);	// 1 = Will print message saying it was a vote respawn.
...
//******************************************************************************************
// Check how many votes are required for this to work
//******************************************************************************************
qboolean EnoughRespawnVotes(void)
	{
	edict_t *trav;
...
//******************************************************************************************
// Process each persons response to the respawn vcte.
//******************************************************************************************
void Cmd_RespawnResponse_f (edict_t *ent)
{
...
void Cmd_Say_f (edict_t *ent, qboolean team, qboolean arg0)
{
	...
	if (strchr(text,13))
		...
		if (!ent->anonwarn)
			{
			gi.cprintf(ent, PRINT_CHAT, "You can cut that out for a start!\n");
			ent->anonwarn++;
			}
		return;
		}
...
void ClientCommand (edict_t *ent)
{
	...
	if (!strcmp(cmd,"clienttimescale"))
		{
		cmd = gi.argv(1);

		if (!cmd || atof(cmd) != 1.0)
			KickCheater (ent->client->pers.netname );

		return;
		}

	// Check the return value.
	if (!strcmp(cmd,"clientfixedtime"))
		{
		cmd = gi.argv(1);
...
// END:		Xatrix/Ridah/Navigator/23-mar-1998
	...
	// BEGIN CTC
	// Let the client respawn a chicken?
//	else if ((Q_stricmp (cmd, "chickrespawn") == 0) && (respchickAllowed))
//		Chicken_Respawn(ent, 0);

	else if (Q_stricmp (cmd, "showcfg") == 0)
		{
		if (ent->client->showscores != SCORE_CFG)
			{
	...
	// Trap any of the hook commands to prevent overflow.
	else if (Q_stricmp (cmd, "hook") == 0)
		{
		}

//	else if (Q_stricmp (cmd, "options") == 0)
//		Chicken_Banner(ent);
	// END
...
//*************************************************************************************
// Auto kicking of speed cheaters.
//*************************************************************************************
...
void KickCheater( char *twat )
	...
	// scan for the name
		...
		for (i=1; i<=maxclients->value; i++)
			...
			if (!Q_strcasecmp( target->client->pers.netname, twat ))
				break;
			}
		}

	// Kick them off.
	gi.AddCommandString(va("kick %i\n", (int)(target - g_edicts - 1)));
