...
qboolean Pickup_Powerup (edict_t *ent, edict_t *other)
{
	...
	// BEGIN CTC
	if (!Chicken_AllowPowerUp(ent, other)) return false;
	// END
...
{
	// BEGIN CTC - JML
	if (!Chicken_CanPickup(other, allowadrenaline)) return false;
	// END
...
{
	// BEGIN CTC
	if (!Chicken_CanPickup(other, allowBigHealth)) return false;
	// END
...
qboolean Pickup_Bandolier (edict_t *ent, edict_t *other)
{
	...
	// BEGIN CTC
	if (!Chicken_CanPickup(other, false)) return false;
	// END
...
void Use_Quad (edict_t *ent, gitem_t *item)
{
	...
	// BEGIN CTC
	if (!Chicken_CanPickup(ent, false)) return;
	// END
...
{
	// BEGIN CTC
//	if (!Chicken_CanPickup(ent, allowInvulnerable)) return;
	// END
...
{
	// BEGIN CTC - JML
	if (!Chicken_CanPickup(ent, allowweaponmods))
		return false;
	// END
...
{
	// BEGIN CTC
	if (!Chicken_CanPickupHealth(ent, other)) return false;
	// END
...
{
	// BEGIN CTC
	if (!Chicken_CanPickup(other, allowArmour)) return false;
	// END
...
void Touch_Item (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (taken)
	{
		...
		// BEGIN CTC
		if (Chicken_ItemTouch(ent, other))
			return;
		else
...
static void drop_make_touchable (edict_t *ent)
{
	...
	// BEGIN CTC
	if (!Chicken_DropMakeTouchable(ent))
		{
		if (deathmatch->value)
			{
