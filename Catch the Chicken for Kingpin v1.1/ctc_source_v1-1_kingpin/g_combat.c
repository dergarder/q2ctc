...
extern int Chicken_TakeDamage(edict_t *self, edict_t *attacker, int damage);

extern int	chickenGame;
extern int	chickenItemIndex;
/*
============
CanDamage
...
// figure momentum add
	...
	if (targ->client && dmg && (!(dflags & DAMAGE_NO_ARMOR)))
	{
	    ...
	    if ((item) && (targ->client->pers.inventory[ITEM_INDEX(item)]))
		{
			...
			{
				// BEGIN CTC - Only reduce the armour if we have a chicken
				if (targ->client->pers.inventory[chickenItemIndex] > 0)
	...
	// BEGIN CTC - Not too sure about this bit but it should work.
	if ((chickenGame && !Chicken_TakeDamage(targ, attacker, damage) && !(dflags & DAMAGE_NO_PROTECTION)) ||
